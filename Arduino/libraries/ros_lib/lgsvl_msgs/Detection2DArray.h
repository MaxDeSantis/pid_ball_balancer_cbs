#ifndef _ROS_lgsvl_msgs_Detection2DArray_h
#define _ROS_lgsvl_msgs_Detection2DArray_h
...
namespace lgsvl_msgs
{
  ...
  class Detection2DArray : public ros::Msg
  {
    public:
      ...
      typedef lgsvl_msgs::Detection2D _detections_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(detections_lengthT > detections_length)
        this->detections = (lgsvl_msgs::Detection2D*)realloc(this->detections, detections_lengthT * sizeof(lgsvl_msgs::Detection2D));
      detections_length = detections_lengthT;
      for( uint32_t i = 0; i < detections_length; i++){
      offset += this->st_detections.deserialize(inbuffer + offset);
        memcpy( &(this->detections[i]), &(this->st_detections), sizeof(lgsvl_msgs::Detection2D));
      }
     return offset;
    }

    const char * getType(){ return "lgsvl_msgs/Detection2DArray"; };
    const char * getMD5(){ return "6104af071edb62623402eb699c3c5b70"; };
