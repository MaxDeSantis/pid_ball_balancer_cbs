#ifndef _ROS_lgsvl_msgs_Detection3DArray_h
#define _ROS_lgsvl_msgs_Detection3DArray_h
...
namespace lgsvl_msgs
{
  ...
  class Detection3DArray : public ros::Msg
  {
    public:
      ...
      typedef lgsvl_msgs::Detection3D _detections_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(detections_lengthT > detections_length)
        this->detections = (lgsvl_msgs::Detection3D*)realloc(this->detections, detections_lengthT * sizeof(lgsvl_msgs::Detection3D));
      detections_length = detections_lengthT;
      for( uint32_t i = 0; i < detections_length; i++){
      offset += this->st_detections.deserialize(inbuffer + offset);
        memcpy( &(this->detections[i]), &(this->st_detections), sizeof(lgsvl_msgs::Detection3D));
      }
     return offset;
    }

    const char * getType(){ return "lgsvl_msgs/Detection3DArray"; };
    const char * getMD5(){ return "463b7bf1d7d2ef45e33f14d3ed064e13"; };
